...
#
#
# A function that performs Bayesian inference of Lotka-Volterra system's parameters from time series data of species populations (the path to data file
# is passed as an argument), using MCMC sampling from unnormalized posterior distribution of parameters.
# The 2-run approach:
...
LV_Bayesian_fit <- function(input.file, output.file, plotting=FALSE, prelim.scale.factor=10, main.scale.factor=1)   #input.file is the path and filename of input data file, output.file is the path and filename of the resulting inferred model (mcmc.chain)
{
  # Check that all required packages are installed. If not, install the missing packages.
  list.of.packages <- c("coda", "deSolve", "Matrix", "mcmc", "adaptMCMC", "runjags") # , "Rmpi") # add Rmpi to use temper.Rmpi to generate the MCMC chains via parallel tempering (for mcmc.method=='temper' only)
  missing.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
  if(length(missing.packages)>0) install.packages(missing.packages,repos="http://cran.csiro.au")
  
  if(!file.exists(input.file)){
    execution.success = FALSE
    error.message = paste("Error: input data file '",input.file,"' does not exist.",sep="")
    return(list(execution.success,error.message))
  } 
  
  if (file.exists(input.file)){  #Input data and learn the model
    if (prelim.scale.factor > 100) {
      prelim.scale.factor = 100
    } else if (prelim.scale.factor < 1) {
      prelim.scale.factor = 1
    }
    
    if (main.scale.factor > 100) {
      main.scale.factor = 100
    } else if (main.scale.factor < 1) {
      main.scale.factor = 1
    }
    
    cat(paste("Preliminary run's chain length:",1e6/prelim.scale.factor,"\n"))
    cat(paste("Main run's chain length:",1e6/main.scale.factor,"\n"))
    
    # Internal functions used in LV_Bayesian_fit_function.R
    #-----------------------------------------------------------------------------------------------------------------------
    ...
    generate_mcmc_chain <- function(mcmc.method,burnin.length,chain.length,thin,params.init)
    {
      ###------------------------------------------------------------------------------------------------------------------------
      if (mcmc.method=="metropolis"){#Metropolis-Hasting-Green method of mcmc sampling
        require(mcmc)
        cat(paste("Burning-in with",burnin.length,"samples..."))
        ...
        cat(paste("done, time elapsed:",mcmc.burnin$time['elapsed'],"seconds \n"))
        cat(paste("Simulating a chain of length",chain.length,"..."))
        ...
        cat(paste("done, time elapsed:",mcmc.chain$time['elapsed'],"\n"))
        cat(paste("Chain acceptance rate:",mcmc.chain$accept,"\n"))
        ...
        for (n in 1:N.species){
          cat(paste("95% confidence interval for r[",n,"] is [",r.mean[n] + (-1)*qnorm(0.975)*r.mcse[n],", ",
                    r.mean[n] + qnorm(0.975)*r.mcse[n],"]",sep='')) 
          cat(paste(" (true value is ",r[n],")\n",sep=''))
        }
        cat("\nMCMC estimates for K:\n")
        for (n in 1:N.species){
          cat(paste("95% confidence interval for K[",n,"] is [",K.mean[n] + (-1)*qnorm(0.975)*K.mcse[n],", ",
                    K.mean[n] + qnorm(0.975)*K.mcse[n],"]",sep=''))
          cat(paste(" (true value is ",K[n],")\n",sep=''))
        }
        cat("\nMCMC estimates for sigma:\n")
        cat(paste("95% confidence interval for sigma is [",sigma.mean + (-1)*qnorm(0.975)*sigma.mcse,", ",
                  sigma.mean + qnorm(0.975)*sigma.mcse,"]",sep=''))
        cat(paste(" (true value is ",sigma.true,")\n",sep=''))
      }
      ###------------------------------------------------------------------------------------------------------------------------
      ...
      if (mcmc.method=="metropolis_adaptive"){#Metropolis-Hasting-Green method of mcmc sampling
        ...
        require(runjags)
        if (mcmc.parallel) {
          cat(paste("Simulating",N.chains,"chains of length",chain.length,"each..."))
          ...
          for (i in 1:length(mcmc.chains)){
            cat(paste("Chain",i,"acceptance rate:",mcmc.chains[[i]]$acceptance.rate,"\n"))
            cat(paste("Maximum log-density found by chain",i,":",max(mcmc.chains[[i]]$log.p),"\n"))
          }
          ...
          mcmc.chains <- MCMC(p=log_uposterior, init=params.init, n=chain.length, scale=scale.vec*scale.factor, adapt=TRUE, acc.rate=0.234, 
                              lb.par=lb.params, ub.par=ub.params)  
          cat(paste("Chain acceptance rate:",mcmc.chains$acceptance.rate,"\n"))
      ...
      if (mcmc.method=="tempering"){
        if (!tempering.parallel){  #single-CPU implementation of parallel tempering
          ...
          cat(paste("Simulating",N.temperatures,"tempered chains of length",chain.length,"each..."))
          ...
          for (slave in names(mcmc.chain.list)){
            cat(paste("Chain from",slave,": within-chain acceptance rate =",mcmc.chain.list[[slave]]$move.accept.rate,
      ...
      ###------------------------------------------------------------------------------------------------------------------------
      return(list('mcmc.chains'=mcmc.chains,'mcmc.chain'=mcmc.chain))
    }
    
    
    ################################################################################################################################################
    # GENERAL PARAMETERS: (do not change unless you know what you are doing)
    ################################################################################################################################################
    import.data <- TRUE  # (default is TRUE) If TRUE, import training data, if FALSE, generate synthetic training data
    data.type = "Volumes"   #either "User-hours" or "Volumes" - specify depending on what we are modelling. Default: "Volumes"
    sampled.fraction = 0.02   # fraction of the total national traffic sampled by DPI
    alpha.positive = TRUE   #should all alphas be positive? (positive alphas => competition, no predator-prey relations)
    alpha.symmetric = FALSE   #should the symmetry of alpha matrix be enforced? (in general it shouldn't)
    Netflix.introduced = FALSE  #introduce Netflix into the system? Default: FALSE
    ...
    # values used to specify the parameter-limiting hyperbox in the parameter space:
    max.param.value <- 3
    eps <- 1e-5
    
    # Optimization algortihm used in optimizer calls optim():
    ...
    Y.series <- try(read.csv(file=input.file, header=T));
    if (class(Y.series)=="try-error"){
      execution.success = FALSE
      error.message = paste("Error reading the input file '", input.file,"' Stopping.",sep="")
      return(list(execution.success,error.message))
      stop(error.message)
    } else {
      cat("successful.\n")
    ...
    N.species <- ncol(Y.series)-1   #number of species in the data
    
    #Adjust the data values for the sampled fraction:
    Y.series[,-1] <- Y.series[,-1]/sampled.fraction
    ...
    Y.series.all.unnormalized <-Y.series.unnormalized <- Y.series   #save the original Y.series before normalizing the data
    max.Y <- max(Y.series[,-1],na.rm=TRUE)  #normalizing constant
    ...
    t.end <- max(t.series)
    dt <- min(min(diff(t.series)),dt.min)   #internal time step used to solve L-V eqs (applies only when ode.solver=='iterate')
    ...
    if (Netflix.introduced) t.switch.on[[N.species]] <- min(Y.series[complete.cases(Y.series[,"Netflix"]),"t"])   # the week at which 1st estimate of Netflix is available in the data file
    t.switch.on <- unlist(t.switch.on)
    
    if (plotting){
      if (data.type=="Volumes"){
        y.label <- "Weekly traffic, KB"
      }
      if (data.type=="User-hours"){
        y.label <- "User-hours"
      }
      plot.dynamics(pop.data=Y.series.all.unnormalized,plot.type='p',pch=1,xlab="Week",ylab=y.label,newplot=T)      #plot observed (noisy) population dynamics
      plot.dynamics(pop.data=Y.series.unnormalized,plot.type='p',pch=19,newplot=F)
    ...
    # Specify the boundaries of the parameter-limiting hyperbox in the parameter space:
    lb.r <- rep(0,N.species)
    ub.r <- rep(max.param.value,N.species)
    #K:
    lb.K <- rep(eps,N.species)
    ub.K <- rep(max.param.value,N.species)
    #alpha:
    if (alpha.positive) lb.alpha <- rep(0,N.species^2-N.species) else lb.alpha <- rep(-max.param.value,N.species^2-N.species)
    ub.alpha <- rep(max.param.value,N.species^2-N.species)
    #sigma:
    lb.sigma <- rep(0,1)
    ub.sigma <- rep(max.param.value,1)
    #X0:
    lb.X0 <- rep(0,N.species)
    ub.X0 <- rep(max.param.value,N.species)
    #overall lb and ub:
    ...
    chain.length <- 1e6/prelim.scale.factor   #length of each mcmc chain
    thin <- 100/prelim.scale.factor           #chain thinning interval
    ...
    #Check that params.init is within the constraints of the parameter hyperbox:
    if (!all(params.init >= lb.params & params.init <= ub.params)) {
      execution.success = FALSE
      error.message = "Error in preliminary run: initial parameters are outside of the parameter hyperbox! Stopping."
      cat(error.message)
      return(list(execution.success,error.message))
    }
    
    # Generate mcmc.chains and the combined mcmc.chain from the preliminary run:
    tmp <- generate_mcmc_chain(mcmc.method,burnin.length,chain.length,thin,params.init)
    mcmc.chains <- tmp$mcmc.chains
    mcmc.chain <- tmp$mcmc.chain
    
    if (plotting) # Some visual diagnostics of the chains:
    {
    ...
    if (plotting){
      ...
      X.series.mle[,-1] <- X.series.mle[,-1]*max.Y
      if (data.type=="Volumes"){
        y.label <- "Weekly traffic, KB"
      }
      if (data.type=="User-hours"){
      ...
      if (import.data==FALSE) {
        plot.dynamics(pop.data=X.series,plot.type='l',xlab="Week",ylab=y.label,newplot=T)   #plot the actual underlying (unobserved) population dynamics
      ...
      } else plot.dynamics(pop.data=X.series.mle,plot.type='l',line.type="dashed",line.width=3,xlab="Week",ylab=y.label,newplot=T)  #plot the inferred population dynamics 
      
      plot.dynamics(pop.data=Y.series.all.unnormalized,plot.type='p',pch=1,newplot=F)      #plot observed (noisy) population dynamics
      plot.dynamics(pop.data=Y.series.unnormalized,plot.type='p',pch=19,newplot=F)
      abline(v=t.start.Y,lty='dotted',lwd=2,col='red')
      abline(v=t.end.Y,lty='dotted',lwd=2,col='red')
    ...
    if (plotting){
      ...
      X.series.optim[,-1] <- X.series.optim[,-1]*max.Y
      if (data.type=="Volumes"){
        y.label <- "Weekly traffic, KB"
      }
      if (data.type=="User-hours"){
      ...
      } else plot.dynamics(pop.data=X.series.optim,plot.type='l',line.type="dashed",line.width=3,xlab="Week",ylab=y.label,newplot=T)  #plot the inferred population dynamics
      plot.dynamics(pop.data=Y.series.all.unnormalized,plot.type='p',pch=1,newplot=F)      #plot observed (noisy) population dynamics
      plot.dynamics(pop.data=Y.series.unnormalized,plot.type='p',pch=19,newplot=F)
      abline(v=t.start.Y,lty='dotted',lwd=2,col='red')
      abline(v=t.end.Y,lty='dotted',lwd=2,col='red')
    ...
    chain.length <- 1e6/main.scale.factor   #length of each mcmc chain
    thin <- 100/main.scale.factor           #chain thinning interval
    ...
    if (!all(params.init >= lb.params & params.init <= ub.params)) {
      execution.success = FALSE
      error.message = "Error in main run: initial parameters are outside of the parameter hyperbox! Stopping."
      cat(error.message)
      return(list(execution.success,error.message))
    }
    
    # Generate mcmc.chains and the combined mcmc.chain from the main run:
    tmp <- generate_mcmc_chain(mcmc.method,burnin.length,chain.length,thin,params.init)
    mcmc.chains <- tmp$mcmc.chains
    mcmc.chain <- tmp$mcmc.chain
    ...
    # We are done! Now, save the mcmc.chain into the specified output file.
    ############################################################################################################################################    
    # Save the combined mcmc.chain from the MAIN run:
    cat(paste("Saving the learned model's MCMC chain to",output.file,"..."))
    result <- try(save_mcmc.chain(mcmc.chain,file.name=output.file))
    if (class(result)=="try-error"){
      execution.success = FALSE
      error.message = paste("Error writing the output file '", output.file,"' Stopping.",sep="")
      return(list(execution.success,error.message))
    } else {
      cat("successful.\n")
    }
    ...
    error.message = "The LV model is learned successfully."
    return(list(execution.success,error.message))
  }
}